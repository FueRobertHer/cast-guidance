...
export function assertHomebrewFile(raw: unknown): {
  ...
  if (meta === null || typeof meta !== "object") {
    throw new ValidationError(
      "missing _meta block — is this a 5etools homebrew file?",
    );
  }
  ...
  for (const s of sources) {
    if (
      typeof s !== "object" ||
      s === null ||
      typeof (s as { json?: unknown }).json !== "string"
    ) {
      throw new ValidationError(
        'every _meta.sources entry needs a string "json" id',
      );
    }
...
export function homebrewEntityCounts(
  json: Record<string, unknown>,
...
export function assertCharacterExport(raw: unknown): CharacterExport {
  ...
  const homebrew = Array.isArray(obj.homebrew)
    ? (obj.homebrew as HomebrewFileRow[])
    : [];
  return {

...
| ID | Remaining work | Acceptance signal |
|---|---|---|
| GAME-001 | Casting the same spell at more than one available slot level now opens an explicit slot/upcast chooser (`availableCastResources` enumerates the whole ladder; `castSpell` takes the chosen resource) instead of always spending the lowest; concentration and action economy are applied in that same `castSpell` call. Exhausted/single-option casts still fire directly (a no-slot cast stays possible). Remaining: surface the same chooser on the Play tab's granted/innate spell casts, show the upcast's stronger effect (damage/target scaling) in the chooser, and treat non-slot resource pools (e.g. sorcery-point conversion) as first-class cast sources. | Every cast path offers the slot/upcast choice and previews the upcast effect; no cast leaves partial side effects. |

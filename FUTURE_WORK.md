...
| GAME-005 | Extend feat/version filtering with source policy and prerequisite guidance. Optional features disable options below the required level, but feats only show a prerequisite string and never flag it (a spell-less Fighter can pick Elemental Adept). Widen `summarizePrerequisite` beyond ability/level/pact/patron/spell to `race`, `feat`, `background`, and `proficiency`; render OR-of-requirement sets as alternatives rather than one flattened AND list; and give feat pickers the same unmet-requirement cue optional features already have. | Feats and invocations show unmet requirements — including race/feat/proficiency and alternative sets — without turning table-approved selections into dead ends. |
...
| ID | Remaining work | Acceptance signal |
|---|---|---|
| FIX-001 | `collectAdditionalSpells` unions every element of an `additionalSpells` array, but that array can hold either level-stages of one grant (correct) or several named, mutually-exclusive branches. Strixhaven Initiate's 15 college entries therefore all resolve — granting roughly a dozen cantrips instead of the two RAW — and its `ability: {choose}` silently defaults to the first option. Detect name-distinguished branches and make them a selection (with the ability prompt). | A feat/trait with N named spell branches grants exactly one chosen branch and prompts for the ability when it is a choice. |
| FIX-002 | 2024 background origin-feat grants that embed a sub-selector (`magic initiate; cleric\|xphb`) are split only on `\|`, so the lookup fails and Acolyte, Sage, and Guide silently lose their granted feat (only a "feat not found" note). Parse the `; <option>` selector, resolve the base feat, and apply the locked sub-choice. | Each 2024 origin feat with a fixed sub-option resolves and applies its effects. |
| FIX-003 | The Play-tab HP −/＋ controls pair a single-tap delta with a double-tap delta, but the browser also fires the two intervening single clicks, so a "double-tap −5" applies −6 and rolls up to three separate concentration saves instead of one at DC = half the total damage. Apply one deterministic amount per gesture and a single save. | A documented tap/double-tap sequence changes HP by exactly the shown amount and triggers one concentration save at the correct DC. |
| FIX-004 | The ASI feat picker offers every feat with no already-taken exclusion and never reads `repeatable`, so a non-repeatable feat taken twice — or granted by a background and then re-picked — double-applies its passive effects (Alert twice = +10 initiative). Honor `repeatable`, exclude non-repeatable duplicates, and dedupe background-vs-ASI copies. | A non-repeatable feat applies at most once; repeatable feats keep independent instances. |
| FIX-005 | Grants that match no reader are dropped or mis-rendered: `expanded` maps keyed by spell level (`s1`–`s5`) parse to `NaN` and vanish entirely (Witherbloom/Lorehold Student), so a whole expanded list is invisible with no note. Represent level-keyed expanded lists the way character-level `expanded` maps already surface as advisory notes. | Level-keyed expanded spell lists surface as notes instead of disappearing. |
...
| UX-004 | Make identity and choice changes non-destructive and reversible. Several actions discard picks instantly with no confirmation — and in the wizard (sessionStorage only) with no undo at all: the Basics rules-version toggle wipes classes/species/background and every choice with no equality guard; picking a class clears all choices doc-wide and drops the subclass; the "change" control on a resolved pick deletes it and every dependent sub-pick; changing or deselecting species/subrace/background prunes their choices; removing a level-1 *secondary* multiclass skips the confirm that single-class removal has; and Standard array / Roll 4d6 overwrite all six scores. Superseded race/class spells and equipment also linger in the doc keyed to the old origin (dormant, and silently re-applied if you switch back). Add equality guards (re-selecting the current option is a no-op), confirm or non-destructively re-scope identity swaps, scope class-change pruning to class choices only, sweep or surface orphaned dormant data, and add an undo affordance (the notices store has no action slot yet). | Re-tapping the current selection changes nothing; every destructive change is confirmed, reversible, or scoped to only the affected picks; a mis-tap is recoverable in both the wizard and the sheet. |
| UX-005 | Make character history navigable and restore predictable — the enabler for UX-004's undo. Entries collapse to generic tokens ("Choices", "Play state", "Equipment", "Spells") with before→after detail only for HP and level, so a player cannot find the snapshot that undoes a specific mis-tap, and Restore shows no preview of what it will change versus the current state. Generate specific, verb-led summaries (which pick changed and its before→after; "Un-prepared Bless"; "Background: Sage → Soldier"), preview the diff before restoring, and coalesce rapid debounced bursts meaningfully. | Each history entry names what changed; restoring previews its differences from the current state before applying. |

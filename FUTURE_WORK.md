...
| GAME-005 | Extend feat/version filtering with source policy and pick-time prerequisite guidance. `summarizePrerequisite` now covers ability/level/pact/patron/spell plus `race`/`feat`/`background`/`proficiency` and renders OR-of-requirement sets as alternatives — but feats still only *show* the string and never flag an unmet requirement (a spell-less Fighter can pick Elemental Adept), whereas optional features already disable options below their required level. Give feat pickers the same advisory unmet-requirement cue (guidance, not a block) and make source policy meaningful in filtering. When that lands, revisit `requiredLevel` (currently the max level across sets) so an OR of alternative sets uses the satisfiable minimum. | Feats and invocations flag unmet requirements — ability/level/race/feat/proficiency and alternative sets — without turning table-approved selections into dead ends. |
